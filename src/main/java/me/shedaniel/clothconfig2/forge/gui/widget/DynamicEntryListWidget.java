...
public abstract class DynamicEntryListWidget<E extends DynamicEntryListWidget.Entry<E>> extends FocusableGui implements IRenderable {
    ...
    protected E getItem(int index) {
        return this.func_231039_at__().get(index);
    }
    ...
    protected final E getItemAtPosition(double mouseX, double mouseY) {
        ...
        return mouseX < (double) this.getScrollbarPosition() && mouseX >= minX && mouseX <= maxX && itemIndex >= 0 && currentY >= 0 && itemIndex < this.getItemCount() ? this.func_231039_at__().get(itemIndex) : null;
    }
    ...
    protected void renderBackBackground(MatrixStack matrices, BufferBuilder buffer, Tessellator tessellator) {
        ...
        buffer.pos(matrix, this.left, this.bottom, 0.0F).tex(this.left / 32.0F, ((this.bottom + (int) this.getScroll()) / 32.0F)).color(32, 32, 32, 255).endVertex();
        buffer.pos(matrix, this.right, this.bottom, 0.0F).tex(this.right / 32.0F, ((this.bottom + (int) this.getScroll()) / 32.0F)).color(32, 32, 32, 255).endVertex();
        buffer.pos(matrix, this.right, this.top, 0.0F).tex(this.right / 32.0F, ((this.top + (int) this.getScroll()) / 32.0F)).color(32, 32, 32, 255).endVertex();
        buffer.pos(matrix, this.left, this.top, 0.0F).tex(this.left / 32.0F, ((this.top + (int) this.getScroll()) / 32.0F)).color(32, 32, 32, 255).endVertex();
        tessellator.draw();
    }
    ...
    public void func_230430_a_(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        Matrix4f matrix = matrices.getLast().getMatrix();
        buffer.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
        buffer.pos(matrix, this.left, this.top + 4, 0.0F).tex(0, 1).color(0, 0, 0, 0).endVertex();
        buffer.pos(matrix, this.right, this.top + 4, 0.0F).tex(1, 1).color(0, 0, 0, 0).endVertex();
        buffer.pos(matrix, this.right, this.top, 0.0F).tex(1, 0).color(0, 0, 0, 255).endVertex();
        buffer.pos(matrix, this.left, this.top, 0.0F).tex(0, 0).color(0, 0, 0, 255).endVertex();
        tessellator.draw();
        buffer.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
        buffer.pos(matrix, this.left, this.bottom, 0.0F).tex(0, 1).color(0, 0, 0, 255).endVertex();
        buffer.pos(matrix, this.right, this.bottom, 0.0F).tex(1, 1).color(0, 0, 0, 255).endVertex();
        buffer.pos(matrix, this.right, this.bottom - 4, 0.0F).tex(1, 0).color(0, 0, 0, 0).endVertex();
        buffer.pos(matrix, this.left, this.bottom - 4, 0.0F).tex(0, 0).color(0, 0, 0, 0).endVertex();
        tessellator.draw();
    ...
    protected void renderScrollBar(MatrixStack matrices, Tessellator tessellator, BufferBuilder buffer, int maxScroll, int scrollbarPositionMinX, int scrollbarPositionMaxX) {
        if (maxScroll > 0) {
            ...
            tessellator.draw();
            buffer.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
            buffer.pos(matrix, scrollbarPositionMinX, int_10 + int_9, 0.0F).tex(0, 1).color(128, 128, 128, 255).endVertex();
            buffer.pos(matrix, scrollbarPositionMaxX, int_10 + int_9, 0.0F).tex(1, 1).color(128, 128, 128, 255).endVertex();
            buffer.pos(matrix, scrollbarPositionMaxX, int_10, 0.0F).tex(1, 0).color(128, 128, 128, 255).endVertex();
            buffer.pos(matrix, scrollbarPositionMinX, int_10, 0.0F).tex(0, 0).color(128, 128, 128, 255).endVertex();
            tessellator.draw();
            buffer.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
            buffer.pos(scrollbarPositionMinX, (int_10 + int_9 - 1), 0.0F).tex(0, 1).color(192, 192, 192, 255).endVertex();
            buffer.pos((scrollbarPositionMaxX - 1), (int_10 + int_9 - 1), 0.0F).tex(1, 1).color(192, 192, 192, 255).endVertex();
            buffer.pos((scrollbarPositionMaxX - 1), int_10, 0.0F).tex(1, 0).color(192, 192, 192, 255).endVertex();
            buffer.pos(scrollbarPositionMinX, int_10, 0.0F).tex(0, 0).color(192, 192, 192, 255).endVertex();
            tessellator.draw();
        }
    }
    ...
    public boolean func_231048_c_(double double_1, double double_2, int int_1) {
        if (this.func_241217_q_() != null) {
            this.func_241217_q_().func_231048_c_(double_1, double_2, int_1);
        }
    ...
    public boolean func_231043_a_(double double_1, double double_2, double double_3) {
        for (E entry : entries) {
            if (entry.func_231043_a_(double_1, double_2, double_3)) {
                return true;
            }
        }
    ...
    protected void moveSelection(int int_1) {
        if (!this.func_231039_at__().isEmpty()) {
            ...
            E itemListWidget$Item_1 = this.func_231039_at__().get(int_3);
    ...
    protected void renderList(MatrixStack matrices, int startX, int startY, int int_3, int int_4, float float_1) {
        ...
        for (int renderIndex = 0; renderIndex < itemCount; ++renderIndex) {
            ...
            if (this.selectionVisible && this.isSelected(renderIndex)) {
                ...
                buffer.pos(matrix, itemMinX, itemY + itemHeight + 2, 0.0F).color(float_2, float_2, float_2, 1.0F).endVertex();
                buffer.pos(matrix, itemMaxX, itemY + itemHeight + 2, 0.0F).color(float_2, float_2, float_2, 1.0F).endVertex();
                buffer.pos(matrix, itemMaxX, itemY - 2, 0.0F).color(float_2, float_2, float_2, 1.0F).endVertex();
                buffer.pos(matrix, itemMinX, itemY - 2, 0.0F).color(float_2, float_2, float_2, 1.0F).endVertex();
                tessellator.draw();
                buffer.begin(7, DefaultVertexFormats.POSITION_COLOR);
                buffer.pos(matrix, itemMinX + 1, itemY + itemHeight + 1, 0.0F).color(0.0F, 0.0F, 0.0F, 1.0F).endVertex();
                buffer.pos(matrix, itemMaxX - 1, itemY + itemHeight + 1, 0.0F).color(0.0F, 0.0F, 0.0F, 1.0F).endVertex();
                buffer.pos(matrix, itemMaxX - 1, itemY - 1, 0.0F).color(0.0F, 0.0F, 0.0F, 1.0F).endVertex();
                buffer.pos(matrix, itemMinX + 1, itemY - 1, 0.0F).color(0.0F, 0.0F, 0.0F, 1.0F).endVertex();
                tessellator.draw();
                RenderSystem.enableTexture();
            }
            ...
            renderItem(matrices, item, renderIndex, y, x, itemWidth, itemHeight, int_3, int_4, this.func_231047_b_(int_3, int_4) && Objects.equals(this.getItemAtPosition(int_3, int_4), item), float_1);
        }
    ...
    protected void renderHoleBackground(MatrixStack matrices, int y1, int y2, int alpha1, int alpha2) {
        ...
        float float_1 = 32.0F;
        buffer.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
        buffer.pos(matrix, this.left, y2, 0.0F).tex(0, ((float) y2 / 32.0F)).color(64, 64, 64, alpha2).endVertex();
        buffer.pos(matrix, this.left + this.width, y2, 0.0F).tex(((float) this.width / 32.0F), ((float) y2 / 32.0F)).color(64, 64, 64, alpha2).endVertex();
        buffer.pos(matrix, this.left + this.width, y1, 0.0F).tex(((float) this.width / 32.0F), ((float) y1 / 32.0F)).color(64, 64, 64, alpha1).endVertex();
        buffer.pos(matrix, this.left, y1, 0.0F).tex(0, ((float) y1 / 32.0F)).color(64, 64, 64, alpha1).endVertex();
        tessellator.draw();
    }
